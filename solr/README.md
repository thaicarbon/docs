...
It is very likely that `solr` does not support the currently selected architecture (`arm32v5`).
...
```console
$ docker run -d -P -e SOLR_HEAP=800m arm32v5/solr
```
...
$ docker run -it -v $PWD/mysolrhome:/opt/mysolrhome -e SOLR_HOME=/opt/mysolrhome arm32v5/solr
```
...
```console
$ docker run -it -v $PWD/mysolrhome:/opt/mysolrhome -e SOLR_HOME=/opt/mysolrhome -e INIT_SOLR_HOME=yes arm32v5/solr
```
...
```console
$ docker run --name solr_custom1 -d -P -v $PWD/custom.sh:/docker-entrypoint-initdb.d/custom.sh arm32v5/solr
$ sleep 5
