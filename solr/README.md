...
It is very likely that `solr` does not support the currently selected architecture (`s390x`).
...
```console
$ docker run -d -P -e SOLR_HEAP=800m s390x/solr
```
...
$ docker run -it -v $PWD/mysolrhome:/opt/mysolrhome -e SOLR_HOME=/opt/mysolrhome s390x/solr
```
...
```console
$ docker run -it -v $PWD/mysolrhome:/opt/mysolrhome -e SOLR_HOME=/opt/mysolrhome -e INIT_SOLR_HOME=yes s390x/solr
```
...
```console
$ docker run --name solr_custom1 -d -P -v $PWD/custom.sh:/docker-entrypoint-initdb.d/custom.sh s390x/solr
$ sleep 5
